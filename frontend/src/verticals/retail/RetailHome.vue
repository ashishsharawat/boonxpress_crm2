  ...
  <div class="h-full overflow-y-auto pb-24">
    ...
    <div class="px-4 py-2">
      <div class="flex items-center justify-between mb-3">
        <h2 class="text-sm font-semibold text-boon-text-primary font-heading">Recent Enquiries</h2>
        <button
      ...
      <EmptyState
        ...
        message="Tap + to log a walk-in or send a catalogue quote."
      />
    </div>

    <div v-if="activeOrders.length || loadingDeals" class="px-4 py-2">
      <div class="flex items-center justify-between mb-3">
        <h2 class="text-sm font-semibold text-boon-text-primary font-heading">Open Orders</h2>
      </div>
    ...
    <SendQuoteSheet v-model="showQuoteSheet" @saved="onOrderSaved" />
  </div>
</template>
...
const businessName = computed(() => config.value?.business_name || config.value?.display_name || 'Retail')
...
async function loadData() {
  ...
  try {
    ...
    newLeads.value = leadsData.map((d) => ({ ...d, doctype: 'CRM Lead', _badge: 'ENQ' }))
    activeOrders.value = dealsData.map((d) => ({ ...d, doctype: 'CRM Deal', _badge: 'ORDER' }))
