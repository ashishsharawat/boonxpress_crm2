<template>
  <div class="h-full overflow-y-auto pb-24">
    ...
    <!-- New Leads (most actionable for a dealer) -->
    <div class="px-4 py-2">
      ...
      <EmptyState
        v-else
        title="No new leads"
        message="Tap + to log a new lead or send a quote."
      />
    </div>
...
import SendQuoteSheet from './SendQuoteSheet.vue'
...
const businessName = computed(() => config.value?.business_name || config.value?.display_name || 'Used Cars')
